...
function may.explode(sep, str)
	sep = tostring(sep)
	if not sep then return 'error' end
	if sep == '' then return may.string_to_table(str) end
...
function may.bf(needle, haystack)
...
function Card:increment(override)
	...
	elseif behavior.random then
		new_rank = pseudorandom_element(rank_data.next, pseudoseed('may_incrementrank'))
	else
...
function Card:decrement(override)
	...
	if behavior.ignore then
		return true
	elseif behavior.random then
		new_rank = pseudorandom_element(rank_data.next, pseudoseed('may_decrementrank'))
	else
...
--ex. may.ctu('Tarot') will get the total number of Tarots used in run
--Not case-sensitive; 'tarot', 'TAROT', 'TaRoT', etc, will work as well
-- Gives the number of ALL consumables if not set is given
function may.ctu(set)
	local count = 0
...
function may.h(name, chip, mul, lv, notif, snd, vol, pit, de)
...
function may.chance(name, probability, absolute)
...
function may.deepcopy(obj, seen)
...
function may.sfavhand()
	if not G.GAME or not G.GAME.current_round then return 'High Card' end
	local chosen_hand = 'High Card'
	local firstmost = may.favhand()
	local _handname, _played, _order = 'High Card', 0, -1 / 0
	for k, v in pairs(G.GAME.hands) do
...
function may.adjacenthands(hand)
	local hands = {}
...
function may.hihand(ignore)
	local chosen_hand = 'High Card'
	local highest_level = 'n/a'
	for _, v in ipairs(G.handlist) do
		if (type(highest_level) == 'string' or G.GAME.hands[v].level > highest_level) and v ~= ignore and SMODS.is_poker_hand_visible(v) then --(Talisman and to_big(highest_level) or highest_level) then
			chosen_hand = v
...
--Gets a random hand
function may.rndhand(ignore, seed, allowhidden)
	local chosen_hand
	ignore = ignore or {}
...
function may.rnd(seed, excluded_flags, pool, ignore_pooling, attempts)
	...
	while true do
		...
		tries = tries - 1
		passes = 0
		selection = G.P_CENTERS[pseudorandom_element(pool or G.P_CENTER_POOLS.Consumeables, pseudoseed(seed or 'mayrnd')).key]
...
function may.rndcard(set, area, seed)
	return create_card(set, area, nil, nil, nil, nil, nil, seed or 'mayrndcard')
end

--Simplified way to create a specific card
function may.card(key)
	if G.P_CENTERS[key] then
		return create_card(G.P_CENTERS[key].set, (G.P_CENTERS[key].set == 'Joker' and G.jokers or G.consumeables), nil, nil, nil, nil, key, 'mayspecificcard')
	end
	return nil
end

--Creates a "raw card"
function may.rawcard(key, sizemul, shiftx, shifty)
...
function Card:resetsize()
	if self.origsize then
		...
		self.children = {}
		self.children.shadow = Moveable(0, 0, 0, 0)
		self:set_sprites(self.config.center, may.bf((self.ability or {}).set or '', resize_lookout) and self.config.card)
		if self.area then
			if (G.shop_jokers and self.area == G.shop_jokers) or (G.shop_booster and self.area == G.shop_booster) or (G.shop_vouchers and self.area == G.shop_vouchers) then
				create_shop_card_ui(self)
			end
