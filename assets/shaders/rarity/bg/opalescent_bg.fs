...
extern MY_HIGHP_OR_MEDIUMP vec2 opalescent_bg;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    float iTime = opalescent_bg.y;

    vec2 uv = (fragCoord * 2.0 - iResolution.xy) / max(iResolution.x, iResolution.y);
    uv *= 1.75 + (uie_scale * 0.00001 + uie_rot * 0.00001);
    vec3 col = vec3(0.0);

    col += grad4(iTime*0.13+(uv.y*-0.0025)+(0.04/(0.4+abs(uv.y))),
        rgb255(168.0*0.8, 255.0*0.8, 120.0*0.8),
        rgb255(120.0*0.8, 255.0*0.8, 214.0*0.8),
        rgb255(10.0, 15.0, 36.0),
        rgb255(11.0, 30.0, 12.0));

    float squareDist = (abs(uv.x)*2.0)+(abs(uv.y*2.0));
    col /= 0.75+0.2*step(0.0, (mod(squareDist-(iTime*0.45), 1.0)-(squareDist*0.35)));
    col *= 0.88+0.12*step(0.2, mod(squareDist-(iTime*0.45), 1.5));
