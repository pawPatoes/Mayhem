...
extern MY_HIGHP_OR_MEDIUMP vec2 paradoxical_bg;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    float iTime = paradoxical_bg.y;

    vec2 uv = (fragCoord * 2.0 - iResolution.xy) / max(iResolution.x, iResolution.y);
    uv *= 1.75 + (uie_scale * 0.00001 + uie_rot * 0.00001);

    uv = vec2(floor(uv.x * 40.0) / 40.0, floor(uv.y * 40.0) / 40.0);

    vec3 col = vec3(0.5);

    col += vec3((0.5 + sin(iTime) * 0.5) * 0.04);

    col += clamp(rand(uv * vec2(iTime)) * 0.18, 0.1, 0.2) - 0.1;

    col += vec3(rand(vec2(iTime + 0.44)), rand(vec2(iTime - 0.12)), rand(vec2(iTime + 1.1)))
           * step(0.998, rand(vec2(iTime, uv.y)));

    col += vec3(rand(vec2(iTime + 0.44)) * 0.4, rand(vec2(iTime - 0.12)) * 0.4, rand(vec2(iTime + 1.1)) * 0.4)
           * step(0.99, rand(vec2(iTime, iTime)));
