...
extern MY_HIGHP_OR_MEDIUMP vec2 demiurgic_bg;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    float iTime = demiurgic_bg.y;

    vec2 uv = (fragCoord * 2.0 - iResolution.xy) / max(iResolution.x, iResolution.y);
    uv *= 1.75 + (uie_scale * 0.0001 + uie_rot * 0.0001);
    vec3 col = vec3(0.0);

    col += grad4(iTime*0.12+(uv.x*0.02)+(uv.y*-0.01),
        rgb255(90.0, 96.0, 190.0),
        rgb255(50.0, 39.0, 160.0),
        rgb255(161.0*0.8, 115.0*0.8, 255.0*0.8),
        rgb255(12.0, 9.0, 41.0));
    ...
    col /= 0.75+0.2*step(0.6, (mod(squareDist-(iTime*0.35), 1.0)-(rotSquareDist*0.35))+(mod(squareDist-(iTime*0.35), 1.0)-(squareDist*0.25)));
    if (rotSquareDist > 3.0) {
