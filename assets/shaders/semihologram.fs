...
extern MY_HIGHP_OR_MEDIUMP vec2 semihologram;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    number offset_l = 0.;
    number offset_r = 0.;
    number timefac = 1.0*semihologram.g;
    ...
    number light_strength = 0.4*(0.3*sin(2.*semihologram.g) + 0.6 + 0.3*sin(semihologram.r*3.) + 0.9);
    vec4 final_col;
    if (tex.a < 0.001){
