...
SMODS.Voucher {	key = 'stardust',	loc_txt = {		name = "Stardust",		text = {			"{C:purple}Poker hands{} gain {C:mult}+5{} Mult and {C:chips}+15{} Chips when",			"{C:planet}leveled up{}"		}	},	pos = { x = 2, y = 1 },	atlas = 'voucher',	cost = 10,	unlocked = true,	calculate = function(self, card, context)		if context.level_up_hand and to_big(context.amount) > to_big(0)	then			may.hand_mod_multchips(context.hand, 'mult', -1, (5 * context.amount), context.instant, context.other_card)			may.hand_mod_multchips(context.hand, 'chips', -1, (15 * context.amount), context.instant, context.other_card)		end	end}

SMODS.Voucher {	key = 'meteor',	loc_txt = {		name = "Meteor",		text = {			"{C:purple}Poker hands{} gain {X:mult,C:white}X1.2{} Mult when",			"{C:planet}leveled up{}"		}	},	pos = { x = 3, y = 1 },	atlas = 'voucher',	cost = 10,	requires = {'v_may_stardust'},	unlocked = true,	calculate = function(self, card, context)		if context.level_up_hand and to_big(context.amount) > to_big(0) then			may.hand_mod_multchips(context.hand, 'mult', 0, (1.2 ^ context.amount), context.instant, context.other_card)		end	end}
